...
```{r}
df_combined <- read.csv(file = "/Users/niharawarawita/Desktop/MSc Project/Data and code/complete_dataset_15subwindows_10slices.csv")
...
## Ratings 1
```{r}
plot1a <- ggplot(df_combined, aes(x = property_name, fill = rating)) +
  geom_bar()  + 
  labs(title='Distribution of the original rating variable across properties ', x='Property', y='Number of occurences')+
  theme(plot.title=element_text(hjust=0.5))+
    scale_fill_manual(values=c("#F8766D","goldenrod2","#7CAE00","#00BE67","#00BFC4","#00A9FF","#C77CFF"))+ ##CD9600"
  guides(fill = guide_legend(title = "Rating"))+
  geom_text(aes(label = ..count..), stat = "count", position = position_stack(vjust = 0.5))
plot1a


plot2a <- ggplot(df_combined, aes(x = property_name, fill = rating_level)) +
  geom_bar()  + 
  labs(title='Distribution of the newly grouped rating variable across properties ', x='Property', y='Number of occurences')+
  theme(plot.title=element_text(hjust=0.5))+
  guides(fill = guide_legend(title = "Rating Level")) +
  scale_fill_discrete(labels = c("Low", "Medium", "High"))+
  scale_fill_manual(values=c("goldenrod2","#53B400","#00B9E3"))+ #C49A00, 00BFC4, 00BFC4
  geom_text(aes(label = ..count..), stat = "count", position = position_stack(vjust = 0.5))
plot2a

```

```{r Fig1, echo=TRUE, fig.height=9, fig.width=10}
grid.arrange(plot1a, plot2a, nrow=2)
```
```{r}
library(scales)
...
gc.grid <- layout(matrix(1:num, nrow = num))
for(i in 1:num){
   gc.ramp <- hue_pal()(i)
   plot(c(0, num), c(0,1),
        type = "n", 
...
plot1b <- ggplot(df_combined, aes(x = property_name, fill = rating)) +
  geom_bar()  + 
...
plot2b <- ggplot(df_combined, aes(x = property_name, fill = rating_level)) +
  geom_bar()  + 
  labs(title='Distribution of rating for each property (combined data)', x='Property', y='Number of occurences')+
  theme(plot.title=element_text(hjust=0.5))+
  guides(fill = guide_legend(title = "Rating Level")) +
